...
Sadece anıyı yaz, başka açıklama ekleme."""
                ...
                # Gemini API çağrısı
                # Önce gemini-1.5-flash dene, olmazsa gemini-pro dene
                try:
                    model = genai.GenerativeModel('gemini-1.5-flash')
                    response = model.generate_content(prompt)
                except Exception as model_error:
                    # gemini-1.5-flash çalışmazsa gemini-pro dene
                    if "not found" in str(model_error).lower():
                        st.warning("⚠️ gemini-1.5-flash bulunamadı, gemini-pro deneniyor...")
                ...
                if "404" in error_msg or "not found" in error_msg:
                    ...
                    st.info("""
                    Bu hata API anahtarınızın eski bir versiyona ait olması veya model isminin değişmiş olmasından kaynaklanabilir.
                    
                    **Çözüm:**
                    1. Yeni bir API anahtarı oluşturun: https://aistudio.google.com/app/apikey
                    2. Eski linkteki anahtarlar artık çalışmıyor olabilir
                    3. Yeni anahtarı Streamlit Secrets'a ekleyin
                    """)
